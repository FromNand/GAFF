##### カーネルのオフセットとサイズが入っているアドレス(LBA方式でセクタ単位) #####
##### この部分を参照してLBA方式でロードすることによって、Make_Partitionプログラムでパーティションとして指定できるようになる #####
.equ	KERNEL_IMAGE_OFFSET, 0x7df6
.equ	KERNEL_IMAGE_SIZE  , 0x7dfa
...
##### プログラムの実行に最低限必要な初期化を行う #####
	...
	ljmpw	$0x0000, $entry			# csを0にしたい。biosからは「ljmpw $0x0000, $0x7c00」でジャンプしているはずだけど、おまじない程度に。
entry:
	movw 	%cs, %ax			# セグメントレジスタを即値で初期化する機械語は存在しないので、axに入れてからセグメントレジスタを初期化している。
	movw	%ax, %ds			# プログラムもデータも同じVMAでアクセスしたいので、dsもcsと同じセグメントを指すようにしておく。
	movw	%ax, %es			# 8086ではセグメントレジスタはcs,ds,es,ssの4つしかなかった。リアルモードでもこの4つのレジスタを初期化。
	movw	%ax, %ss			# ssのベースは0。オフセットのspでスタックトップを決める。
	movw	$0x7c00, %sp			# スタックポインタはbiosがIPLをロードする0x7c00のすぐ下に設定する。
...
disk_read_next:
	...
	ljmpw	$0x0000, $0x8000		# 0x8000番地にジャンプする。
fin:
	hlt					# 楽しいHLT命令。
	jmp	fin				# IFがクリアされている状態では、HLTが割り込みとかで抜けることがあるみたい(?)だから一応無限ループにしておく。

##### 文字列表示関数 #####
putloop:
	lodsb					# 文字列から一文字読み込む。
	cmpb	$0, %al				# それがなる文字かどうかのチェック。
	je	putloop_end			# なる文字だったら無限ループ。
...
segm:
	.word	0x0800				# 読み込み先のセグメントベースを指定する。	
lba0:
